...
export default function Home() {
  ...
  async function getJokes() {
    const creatorJokes = await axios.get(
      `/api/creator/${params.name}`
    );
    const { creator } = creatorJokes.data;

    const rJoke: string = creator.piadas[getRandomIndex(creator.piadas)];

    const randomJokeCreator = await axios.get(
      `/api/joke/${rJoke}`
    );

    const randomJoke: Joke = randomJokeCreator.data.j;
    setJoke(randomJoke);
    ...
    setJoke(randomJoke);
    const cre = await axios.get(
      `/api/creator/${randomJoke.creator}`
    );
